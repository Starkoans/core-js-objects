...
function makeWord(lettersObject) {
  const wordObj = Object.entries(lettersObject).reduce((acc, curr) => {
    const [letter, [...positions]] = curr;
  ...
  return Object.values(wordObj).join('');
}
    ...
    (cash, bill) => {
      ...
      switch (bill) {
        ...
        case 50:
          ...
          break;
        case 100:
          if (updatedCash[25] > 0 && updatedCash[50] > 0) {
            updatedCash[100] += 1;
            updatedCash[25] -= 1;
            updatedCash[50] -= 1;
          } else if (updatedCash[25] >= 3) {
            updatedCash[25] -= 3;
            updatedCash[100] += 1;
          } else flag = false;
          break;
...
class CssSelector {
  ...
  setSelector(selectorKey, order, value) {
    if (this.order > order) {
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    }
    if (Array.isArray(this.selectorParts[selectorKey])) {
      this.selectorParts[selectorKey].push(value);
    } else {
      if (this.selectorParts[selectorKey]) {
        throw new Error(
          'Element, id and pseudo-element should not occur more then one time inside the selector'
        );
      }
      this.selectorParts[selectorKey] = value;
    }
    this.order = order;
    return this;
  }
  ...
  element(value) {
    return this.setSelector('element', 0, value);
  }

  id(value) {
    return this.setSelector('id', 1, `#${value}`);
  }

  class(value) {
    return this.setSelector('classes', 2, `.${value}`);
  }

  attr(value) {
    return this.setSelector('attributes', 3, `[${value}]`);
  }

  pseudoClass(value) {
    return this.setSelector('pseudoClasses', 4, `:${value}`);
  }

  pseudoElement(value) {
    return this.setSelector('pseudoElement', 5, `::${value}`);
  }
